...
use App\Models\Tailoring\OrderPayment;
use App\Models\Tailoring\TailorAssignment;
...
class OrderRepo extends BaseRepository
{
    ...
    public function getData(Request $request): LengthAwarePaginator
    {
        ...
        $sortable = ['order_number', 'total_amount', 'expected_delivery_date', 'created_at'];
    ...
    public function completeOrder(Order $order, ?int $userId): Order
    {
        return DB::transaction(function () use ($order, $userId) {
            $order = $this->model->with('items.materials.product')->lockForUpdate()->findOrFail($order->id);
